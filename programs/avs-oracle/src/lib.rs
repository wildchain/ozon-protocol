...
declare_id!("GER4ctswvnc9TjBj98wA9FX1gMnQ4vmFSxNrAJZvWQXt");
...
pub mod avs_oracle {
    ...
    msg!("TASK CREATED WITH VALUES: Task {} created by AVS {}", task_id, task.avs);
        
    Ok(())
 
    }

    pub fn submit_task_result(ctx: Context<SubmitTaskResult>, submitted_price: i64, confidence: u64, publish_time: i64,) -> Result<()> {
        let task = &mut ctx.accounts.task_account;
        ...
        let current_slot = Clock::get()?.slot;
        require!(current_slot <= task.submission_deadline,ErrorCode::TaskExpired);
        ...
        require!(operator_account.bond_amount >= 1_000_000_000,  ErrorCode::InsufficientBond);
        require!(operator_avs_reg.tasks_failed < 5, ErrorCode::TooManyFailures);
        ...
        emit!(TaskSubmittedEvent {task_id: task.task_id, operator: submission.operator, submitted_price,submitted_slot: current_slot});
        
        msg!(" SUBMISSION OF TASK DONE : Operator {} submitted result for task {}", submission.operator, task.task_id);
        
        Ok(())
    }

    pub fn verify_and_slash_if_wrong(ctx: Context<VerifyAndSlashIfWrong>, operator_owner: Pubkey,maximum_age: u64,) -> Result<()> {
        ...
        if !is_correct {
            ...
            let operator_account = &ctx.accounts.operator_account;
            let slash_amount = operator_account.bond_amount / 10; // 10% slash
            
            // Preparing CPI context
            ...
            // Calling the slash using cpi in my other contract 
            restaking_programs::cpi::slash_operator(
                cpi_ctx,
                operator_owner,
                slash_amount,
            )?;
            
            msg!(" Operator slashed {} lamports and rewards are now channeled to impact treasury", slash_amount);
        } else {
    ...
    pub fn close_task(ctx: Context<CloseTask>) -> Result<()> {
        ...
        msg!("Task {} closed. Submissions: {}, Verified: {}", 
             task.task_id, task.total_submissions, task.verified_submissions);
...
pub struct CreateTask<'info> {
    ...
    #[account(
        ...
        seed = [b"task", avs_owner.key().as_ref(), task_id.to_le_bytes().as_ref()],
        bump
    )]
...
pub struct SubmitTaskResult<'info> {
    ...
    /// Reference to restaking program
...
pub struct CloseTask<'info> {
    ...
    #[account(
        mut,
        constraint = task_account.avs == avs_owner.key() @ ErrorCode::Unauthorized //doubt - shouldn't i close this?
    )]
    pub task_account: Account<'info, TaskAccount>,
}
